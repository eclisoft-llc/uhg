...
#               2. Database Name
#               3. Schema Name
#               4. Full path of folder/file to be transfer
#               5. Internal stage name
#
...
do
  ...
  snowsql -c ${sf_env}  -f ${Log_dir}/${line}.sql >> $Log_Dir1 2>&1
  if [ $? -eq 0 ]
  then
...
#Validating the parameters for folder/file push
if [ -d "${fd_name}" ]; then
