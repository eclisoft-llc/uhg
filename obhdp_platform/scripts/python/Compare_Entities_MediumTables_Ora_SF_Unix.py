...
# Date: 05/21/2021
# Description: Compare a data set from Oracle against Snowflake, document
# the differences in large tables like dim_member, vendor, provider
# 08/23/2021 BBalboa - added the entity-attr list for DIMs, code clean-up
...
with open(r'/obhdp_dev/tcoc/d_scripts/python_tcoc.yaml') as file:
    config = yaml.load(file, Loader=yaml.FullLoader)

print(config)
snowFlakeInfo = config['accountname']
sandschema=config['schemaname']
...
print("Key path:--->", config['private_key_path'])


with open (config['private_key_path'], "rb") as key:
    p_key= serialization.load_pem_private_key(
        key.read(),
        password=config['passwordphrase'].encode(),
...
diff_path = '/obhdp_dev/tcoc/d_outbound/'
    ...
    for entity, attribute_list in input_list.items():
        ...
        while day <= endday:
            try:
                ...
                q1 = 'SELECT ' + listToStr + ' FROM ' + entity + ' WHERE EXTRACT(YEAR FROM DW_INSERT_DATETIME) = ' \
