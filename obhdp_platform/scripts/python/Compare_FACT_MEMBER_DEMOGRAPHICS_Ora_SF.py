...
input_list = {
    "FACT_MEMBER_DEMOGRAPHICS": ["TO_CHAR(MEMB_DEMOGRAPHICS_FULL_DATE,'YYYY-MM-DD')","MEMB_DIM_ID","PLAN_DIM_ID","COMPANY_DIM_ID","AGE","MEMBER_CNT","DW_INSERT_DATETIME","DW_UPDATE_DATETIME","PROV_DIM_ID","GEOGRAPHY_DIM_ID","LINE_OF_BUSINESS_DIM_ID","ELIG_STATUS_DIM_ID","GROUP_DIM_ID","PRODUCT_DIM_ID","CONVERTED_FROM_DIM_ID","COSMOS_CUST_SEG_DIM_ID","CARE_LEVEL_DIM_ID","CASE_MANAGER_DIM_ID","COMORBIDITY_CNT","PLAN_LOB_DIM_ID","MONTH_DIM_ID","MEDICARE_TYPE_DIM_ID","LTC_IND_DIM_ID","UNISON_PRODUCT_DIM_ID","STATE_SPEC_COUNTY_DIM_ID","RTCL_RATE_DIM_ID","SYS1","SYS2","SYS3","SYS4","SOURCE_PRODUCT_DIM_ID","CLASS_DIM_ID","CLASS_PLAN_DIM_ID","COA_DIM_ID","PROGRAM_STATUS_DIM_ID","SDA_DIM_ID"]
}
    ...
    for entity, attribute_list in input_list.items():
        ...
        while day <= endday:
            try:
                ...
                q1 = 'SELECT ' + listToStr + ' FROM ' + entity + ' WHERE EXTRACT(YEAR FROM MEMB_DEMOGRAPHICS_FULL_DATE) = ' \
                     + str(yearNumber) + ' AND EXTRACT(MONTH FROM MEMB_DEMOGRAPHICS_FULL_DATE) = ' + str(month) \
                     + ' AND EXTRACT(DAY FROM MEMB_DEMOGRAPHICS_FULL_DATE) = ' + str(day) \
                     + ' ORDER BY MEMB_DEMOGRAPHICS_FULL_DATE,MEMB_DIM_ID,PLAN_DIM_ID,COMPANY_DIM_ID,PROV_DIM_ID,GEOGRAPHY_DIM_ID LIMIT ' + rowcomparison
                q2 = 'SELECT ' + listToStr + ' FROM DW.' + entity + ' WHERE EXTRACT(YEAR FROM MEMB_DEMOGRAPHICS_FULL_DATE) = ' \
                     + str(yearNumber) + ' AND EXTRACT(MONTH FROM MEMB_DEMOGRAPHICS_FULL_DATE) = ' + str(month) \
                     + ' AND EXTRACT(DAY FROM MEMB_DEMOGRAPHICS_FULL_DATE) = ' + str(day) \
                     + ' ORDER BY MEMB_DEMOGRAPHICS_FULL_DATE,MEMB_DIM_ID,PLAN_DIM_ID,COMPANY_DIM_ID,PROV_DIM_ID,GEOGRAPHY_DIM_ID FETCH FIRST ' + rowcomparison +' ROWS ONLY'
